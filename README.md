...
1. `.venv/Scripts/python.exe src/run_pipeline.py --skip-collect --input-csv data/output/x_recent_search_20260509_173241.csv --post-new-events --post-dry-run`
...
- 現在の GitHub Actions は投稿処理を呼んでいないため、GitHub 側にこれらの Secrets はまだ不要です
- 将来 Actions から自動投稿する場合だけ、同じ 4 つの値を GitHub Secrets に追加し、workflow から `--post-new-events` を有効にします
