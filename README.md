...
このリポジトリには GitHub Actions 用 workflow を追加できます。API 版 collector を使って定期実行し、公開用 JSON を自動更新して push する想定です。

必要な Secrets / Variables:

- `X_BEARER_TOKEN`: X recent search API 用 Bearer Token
- `GH_MODELS_TOKEN`: GitHub Models 抽出用トークン
- `GITHUB_MODELS_MODEL` (optional): 既定モデルを上書きしたい場合の repository variable
- `GITHUB_MODELS_API_VERSION` (optional): API version を上書きしたい場合の repository variable

workflow の内容:

- `src/build_priority_queries_from_masters.py` でクエリ設定を再生成
- `src/run_pipeline.py --collector api` を実行
- `config/priority_queries.json`、`docs/data/schedule_list.json`、`docs/data/master_data.json` に差分があれば commit / push

追加後は GitHub の Actions タブから手動実行でき、schedule でも毎日自動実行されます。
